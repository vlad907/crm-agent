...
import { ApiError, convertProspectsToLeads, getProspects, runProspectSearch } from "@/src/lib/api";
...
const PAGE_SIZE = 25;
const SEARCH_PRESETS: Array<{ label: string; value: string }> = [
...
export default function ProspectsPage() {
  ...
  const [runLocation, setRunLocation] = useState("39.727132,-121.843275");
  const [runRadius, setRunRadius] = useState("10000");
  ...
  async function onRunSearch(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const categories = runCategories
      ...
      .map((part) => part.trim())
      .filter(Boolean);
    const radius = Number.parseInt(runRadius, 10);
  ...
  function onSkipSelected(): void {
    if (!selectedIds.size) {
      setError("Select prospects to skip.");
      return;
    }
    const skippedCount = selectedIds.size;
    setSelectedIds(new Set());
    setActionMessage(`Skip queued for ${skippedCount} selected prospects (backend endpoint coming soon).`);
  }
      ...
      <section className="card surface-search stack">
        ...
        <form className="stack" onSubmit={onRunSearch}>
          <div className="row">
            ...
            <div className="field">
              <label htmlFor="search_location">Location (lat,lng)</label>
              <input
                id="search_location"
                value={runLocation}
                onChange={(event) => setRunLocation(event.target.value)}
                placeholder="39.727132,-121.843275"
              />
            </div>
            <div className="field">
              <label htmlFor="search_radius">Radius (meters)</label>
              <input id="search_radius" value={runRadius} onChange={(event) => setRunRadius(event.target.value)} />
            </div>
          </div>
        ...
        {selectedIds.size > 0 ? (
          <div className="bulk-action-bar">
            ...
            <div className="inline-actions">
              ...
              <button className="btn-secondary" type="button" onClick={onSkipSelected}>
                Skip
              </button>
              <button className="btn-secondary" type="button" onClick={onExportCsv}>
                Export CSV
              </button>
              <button className="btn-danger" type="button" disabled title="Coming soon">
                Remove Prospect
              </button>
            </div>
          </div>
